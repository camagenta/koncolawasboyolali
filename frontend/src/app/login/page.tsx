...
const errorMessages: Record<string, string> = {
  no_token: 'Token tidak ditemukan. Silakan coba login kembali.',
  invalid_data: 'Data pengguna tidak valid. Silakan coba login kembali.',
...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (errCode && errorMessages[errCode]) {
      setError(errorMessages[errCode])
    }
  }, [searchParams])
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4">
      <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-sm text-center">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">IKASMANSA</h1>
          <p className="text-sm text-gray-500 mt-1">Ikatan Alumni SMAN 1 Boyolali</p>
        </div>

        {error && (
          <div className="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-sm text-red-700 text-left">
        ...
        <p className="mt-6 text-xs text-gray-400">
          Hanya untuk alumni & pengurus SMAN 1 Boyolali
        </p>
      </div>
    </div>
  )
